                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        <p><strong>Nama:</strong><br>
                            {{ $terlambat->pegawai->gelar_dpn ?? '' }}{{ $terlambat->pegawai->gelar_dpn ? ' ' : '' }}{{ $terlambat->pegawai->nama }}{{ $terlambat->pegawai->gelar_blk ? ', ' . $terlambat->pegawai->gelar_blk : '' }}
                        </p>
                        <p><strong>NIP:</strong><br> {{ $terlambat->pegawai->nip }}</p>
                        <p><strong>Jenis Ijin:</strong><br> {{ $terlambat->jenis_ijin }}</p>
                        <p><strong>Status Saat Ini:</strong><br>
                            <span class="fw-bold text-primary">{{ ucfirst($terlambat->status) }}</span>
                        </p>
                    </div>
                    <div class="col-md-6">
                        <p><strong>Tanggal terlambat:</strong><br>
                            {{ date('d M Y', strtotime($terlambat->tanggal)) }}
                        </p>
                        <p><strong>Disetujui Atasan:</strong><br>
                            {{ $terlambat->tanggal_disetujui_pejabat ? date('d M Y: H:i:s', strtotime($terlambat->tanggal_disetujui_pejabat)) : '-' }}
                        </p>
                    </div>
                </div>

                @php
                    $status = strtolower($terlambat->status);
                    $steps = ['Diajukan', 'Diproses', 'Disetujui'];
                    $currentIndex = array_search(ucfirst($terlambat->status), $steps);
